...
$(function () {
  ...
  btnRoll.on("click", () => {
    ...
    if (toRoll > 0) {
      ...
      if (sums["T"].slice(0, 1)) {
        $("#column-total-digit-0").attr("src", `assets/val${sums["T"].slice(0, 1)}.png`).show();

      }
      if (sums["T"].slice(1, 2)) {
        $("#column-total-digit-1").attr("src", `assets/val${sums["T"].slice(1, 2)}.png`).show();
      }
      if (sums["T"].slice(2, 3)) {
        $("#column-total-digit-2").attr("src", `assets/val${sums["T"].slice(2, 3)}.png`).show();
      }
